...
{
    int codle,matrie;
    char data_sistema[15],data_dev[15];
} EMPRESTIMO;
...
void emprestimo(EMPRESTIMO *pt,LIVRO *ptr, ALUNO *pp)
{
    ...
    while (fread(pp,sizeof(*pp),1,file)==1)
    {
        if(e == pp->matri)
        {
            ...
            printf("NOME DO ALUNO: %s",pp->nomeal);
            gotoxy(34,10);
            printf("CPF: %s",pp->cpf);
            gotoxy(34,12);
            system("pause");
    ...
    else
    {
        ...
        else
        {
            ...
            while(fread(ptr,sizeof(*ptr),1,file)==1)
            {
                ...
                {
                    i++;
                    printf("NOME DO LIVRO: %s",ptr->nomel);
                    gotoxy(34,10);
                    printf("NOME DO AUTOR: %s",ptr->nomea);
                    gotoxy(34,11);
                    printf("AREA DO CONHECIMENTO: %s",ptr->area);
                    gotoxy(34,13);
                    g= ptr->quant;
                }
            }
